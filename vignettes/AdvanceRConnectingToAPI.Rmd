...
apiCall <- AdvanceRConnectingToAPI$new(kpi = list("N25026"), year = list("2020"))
```
...
apiCall <- AdvanceRConnectingToAPI$new(searchText = "Stockholm", searchTextIsKPI = FALSE)
```
...
apiCall <- AdvanceRConnectingToAPI$new(kpi = list("N25026"), year = list("2020", "2022"))
print(apiCall$fetchedData)
```
...
apiCall <- AdvanceRConnectingToAPI$new(searchText = "miljö", searchTextIsKPI = TRUE, sweText = TRUE)
print(apiCall$fetchedData)
```

You can also perform the search in English by setting `sweText = FALSE`:

```{r}
# Example: Search for KPIs in English
apiCall <- AdvanceRConnectingToAPI$new(searchText = "environment", searchTextIsKPI = TRUE, sweText = FALSE)
```
